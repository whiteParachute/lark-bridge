...
5. `Session.backend` 是 `Backend` 接口实例，由 `createBackend(kind)` 工厂产出（`backend/index.ts`）。三种实现：
   ...
   - `TmuxBackend` (`backend/tmux.ts`) — 不走 SDK；启动/attach 后先只读观察当前 pane 输出，飞书侧普通消息会先剥掉末尾 CR/LF；单行输入用 `send-keys -l` 逐字输入后提交，多行输入才用 `paste-buffer`，再轮询 `capture-pane`。关闭 bridge session 时不 kill tmux，支持电脑端和飞书端交替接管同一 codex/claude code session。
6. 后端流式事件分发到两类卡片：
