...
| `/tmux new <session> [claude\|codex] [cwd]` | tmux 启用后可用；创建或接管一个 tmux session；不存在则在 `cwd` 启动对应 CLI，存在则直接接管 |
| `/tmux attach <session[:window[.pane]]>` | tmux 启用后可用；接管已有 tmux target；后续普通消息会 paste 到该 pane |
...
tmux 后端没有 SDK turn-complete 事件，只能在 pane 输出静默 `settleDelayMs` 后认为本轮完成；长任务运行时不要连续发送多条普通消息，避免把输入 paste 到仍在执行的 TUI。
...
tmux 后端不走 SDK，也不解析真实 turn 完成事件。它把消息 paste 到 pane 后轮询 `capture-pane`，在 `settleDelayMs` 时间内无新输出时把本轮视为完成；关闭 bridge session 时不会向 pane 发送 `C-c`，避免误杀电脑端正在运行的任务。
